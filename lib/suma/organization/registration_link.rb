...
# Table: organization_registration_links
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Columns:
...
#  ical_dtstart     | timestamp with time zone |
#  ical_dtend       | timestamp with time zone |
#  ical_rrule       | text                     | NOT NULL DEFAULT ''::text
#  search_content   | text                     |
#  search_embedding | vector(384)              |
#  search_hash      | text                     |
# Indexes:
#  organization_registration_links_pkey                          | PRIMARY KEY btree (id)
#  organization_registration_links_opaque_id_key                 | UNIQUE btree (opaque_id)
#  organization_registration_links_organization_id_index         | btree (organization_id)
#  organization_registration_links_search_content_tsvector_index | gin (to_tsvector('english'::regconfig, search_content))
# Foreign key constraints:
#  organization_registration_links_created_by_id_fkey   | (created_by_id) REFERENCES members(id) ON DELETE SET NULL
#  organization_registration_links_intro_id_fkey        | (intro_id) REFERENCES translated_texts(id)
#  organization_registration_links_organization_id_fkey | (organization_id) REFERENCES organizations(id) ON DELETE CASCADE
# Referenced By:
#  organization_memberships | organization_memberships_registration_link_id_fkey | (registration_link_id) REFERENCES organization_registration_links(id) ON DELETE SET NULL
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
