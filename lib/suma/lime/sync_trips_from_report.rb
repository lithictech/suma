...
class Suma::Lime::SyncTripsFromReport
  ...
  def run
    ...
    ds = ds.select(
      ...
      :timestamp,
      :subject,
      Sequel.pg_jsonb(:data).get("Attachments").as(:attachments),
    )
  ...
  def dataset
    ...
    ds = Suma::Webhookdb.postmark_inbound_messages_dataset.
      ...
      where(Sequel.like(Sequel.pg_jsonb(:data).get_text("To"), "%#{Suma::Lime.trip_report_to_email}%")).
  ...
  def parsetime(t)
    if /\d\d\d\d \d\d:\d\d:\d\d GMT[+-]\d/.match?(t)
      ...
      # Remove a potential timezone parenthetical
      trimmed = t[/\A.*?GMT[+-]\d+/]
      # Strict parse, Time.parse doesn't find the timezone offset.
