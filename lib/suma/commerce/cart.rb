...
class Suma::Commerce::Cart < Suma::Postgres::Model(:commerce_carts)
  ...
  one_to_many :items, class: "Suma::Commerce::CartItem", order: order_desc
  one_to_many :checkouts, class: "Suma::Commerce::Checkout", order: order_desc
...
#  commerce_carts_member_id_fkey   | (member_id) REFERENCES members(id) ON DELETE CASCADE
#  commerce_carts_offering_id_fkey | (offering_id) REFERENCES commerce_offerings(id)
# Referenced By:
#  commerce_cart_items | commerce_cart_items_cart_id_fkey | (cart_id) REFERENCES commerce_carts(id) ON DELETE CASCADE
