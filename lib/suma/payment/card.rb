...
class Suma::Payment::Card < Suma::Postgres::Model(:payment_cards)
  ...
  def refetch_remote_data
    ...
    existing = Stripe::Customer.retrieve_source(customer, self.stripe_id)
...
#  search_hash      | text                     |
#  expires_at       | timestamp with time zone | DEFAULT timezone('UTC'::text, (make_timestamp(((stripe_json ->> 'exp_year'::text))::integer, ((stripe_json ->> 'exp_month'::text))::integer, 1, 0, 0, (0)::double precision) + '1 mon'::interval))
#  last4            | text                     | DEFAULT (stripe_json ->> 'last4'::text)
#  brand            | text                     | DEFAULT (stripe_json ->> 'brand'::text)
#  name             | text                     | DEFAULT (((stripe_json ->> 'brand'::text) || ' x-'::text) || (stripe_json ->> 'last4'::text))
# Indexes:
#  payment_cards_pkey                          | PRIMARY KEY btree (id)
#  payment_cards_legal_entity_id_index         | btree (legal_entity_id)
#  payment_cards_search_content_trigram_index  | gist (search_content)
#  payment_cards_search_content_tsvector_index | gin (to_tsvector('english'::regconfig, search_content))
