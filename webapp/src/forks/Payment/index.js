...
cardFromNumber = function (num) {
  let card, foundCard, j, len, match;
  num = (num + "").replace(/\D/g, "");
  foundCard = void 0;
...
formatCardNumber = function (maxLength) {
  return function (e) {
    let card, digit, i, j, len, length, re, target, upperLength, upperLengths, value;
    if (e.which > 0) {
...
restrictCardNumber = function (maxLength) {
  return function (e) {
    let card, digit, length, target, value;
    target = e.target;
    digit = String.fromCharCode(e.which);
    if (!/^\d+$/.test(digit)) {
      return;
...
setCardType = function (e) {
  let allTypes, card, cardType, target, val;
  target = e.target;
  val = QJ.val(target);
