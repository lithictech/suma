...
RSpec.describe Suma::Lime::MaasClient, :db do
  let(:client) { described_class.new("authtok") }
  ...
  it "gets vehicle details" do
    ...
    resp = client.get_vehicle(qr_code_json:, license_plate:)
    expect(req).to have_been_made
    expect(resp).to include("data" => hash_including(
  ...
  it "creates a new user successfully" do
    ...
    resp = client.create_user(phone_number:, email_address:, driver_license_verified:)
    expect(req).to have_been_made
    expect(resp).to include("data" => hash_including(
