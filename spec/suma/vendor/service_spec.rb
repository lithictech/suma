...
RSpec.describe "Suma::Vendor::Service", :db do
  ...
  describe "datasets" do
    it "can find rows available to a member based on constraints" do
      ...
      expect(described_class.available_to(mem_no_constraints).all).to have_same_ids_as(no_constraint)
      expect(described_class.available_to(mem_verified_constraint).all).to have_same_ids_as(
        no_constraint,
        with_constraint,
      )
      expect(described_class.available_to(mem_rejected_constraint).all).to have_same_ids_as(no_constraint)
    end
  end
