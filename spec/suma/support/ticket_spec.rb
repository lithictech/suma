...
RSpec.describe "Suma::Support::Ticket", :db do
  ...
  describe "#sync_to_front" do
    it "creates a discussion conversation" do
      ...
      req = stub_request(:post, "https://api2.frontapp.com/conversations").with do |r|
        expect(r.headers).to include("Content-Type" => start_with("multipart/form-data; boundary=--"))
        expect(r.body).to include("Content-Disposition: form-data; name=\"type\"\r\n\r\ndiscussion")
        expect(r.body).to include("name=\"inbox_id\"\r\n\r\ninb_123")
        expect(r.body).to include("name=\"subject\"\r\n\r\nHello")
    ...
    it "includes attached uploaded files" do
      ...
      req = stub_request(:post, "https://api2.frontapp.com/conversations").with do |r|
        ...
        expect(r.body).to include("name=\"inbox_id\"\r\n\r\ninb_123")
        expect(r.body).to include("name=\"subject\"\r\n\r\nHello")
        expect(r.body).to include("name=\"attachments[0]\"; filename=\"im.html\"\r\nContent-Type: text/html\r\n\r\nxyz")
      end.to_return(fixture_response("front/conversation_create"))
